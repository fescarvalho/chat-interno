...
const STATUS_CONFIG: Record<string, { color: string; label: string }> = {
  ...
  AWAY:   { color: "bg-yellow-400",  label: "Ausente" },
...
export function Sidebar() {
  ...
  // Filtra contatos pelo texto digitado na busca
  const filteredUsers: UserProfile[] = searchQuery.trim()
    ? usersList.filter(
        (c) =>
  ...
  return (
    <aside className="w-80 flex-shrink-0 border-r border-indigo-900/50 bg-slate-900 text-slate-100 flex flex-col h-full">
        ...
        <div className="flex items-center">
          <div className="relative">
            <Avatar className="h-10 w-10 border border-indigo-500/50 shadow-sm">
              <AvatarFallback className="bg-indigo-600 text-white">
                {currentUserProfile?.name?.substring(0, 2).toUpperCase() ||
          ...
          <div className="ml-3 flex flex-col max-w-[140px]">
            <span className="text-sm font-semibold truncate text-white">
              {currentUserProfile?.name || "Carregando..."}
            </span>
        ...
        <div className="p-3 space-y-1">
          ...
          {filteredUsers.length === 0 ? (
            ...
            filteredUsers.map((contact) => {
              const statusCfg =
                STATUS_CONFIG[contact.status] || STATUS_CONFIG.OFFLINE;
              const unread = unreadByUserId[contact.id] || 0;

              return (
                <button
                  key={contact.id}
                  onClick={() =>
                    user && startDirectChat(user.id, contact)
                  }
                ...
                >
                  <div className="relative">
                    ...
                    {/* Bolinha de status (atualiza em tempo real) */}
                    <div
                      className={`absolute bottom-0 right-0 w-3.5 h-3.5 rounded-full border-2 border-slate-900 transition-colors duration-500 ${statusCfg.color}`}
                    />
                  </div>
      ...
      {/* ── Footer com versão correta ─────────────────────────────────────── */}
      <div className="p-3 border-t border-white/10 text-center">
        <span className="text-[10px] font-medium text-slate-500 uppercase tracking-widest">
          ChatPC v{APP_VERSION}
        </span>
      </div>
    </aside>
  );
